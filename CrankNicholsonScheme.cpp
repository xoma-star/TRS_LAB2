...
double CrankNicholsonScheme::calculate(const bool isSaveGpaph, const bool isSaveError)
{
	...
	for (unsigned t = 0; t < numberOfStepsT; ++t) {
		...
		for (unsigned x = 1; x < numberOfStepsX; ++x) {
			//верхн.
			elementsMatrix[0 * (numberOfStepsX + 1) + x] = gridPitchT * k_0 / (2. * gridPitchX * gridPitchX);

			//ср.
			elementsMatrix[1 * (numberOfStepsX + 1) + x] = -1 - gridPitchT * k_0 / (gridPitchX * gridPitchX);

			//нижн;
			elementsMatrix[2 * (numberOfStepsX + 1) + x - 1] = gridPitchT * k_0 / (2. * gridPitchX * gridPitchX);

			//прав;
			elementsMatrix[3 * (numberOfStepsX + 1) + x] = -layers[t * (numberOfStepsX + 1) + x] -
				gridPitchT * f(gridPitchT * (t + .5), x * gridPitchX) -
				gridPitchT * k_0 * .5 / (gridPitchX * gridPitchX) *
				(
					layers[t * (numberOfStepsX + 1) + x - 1] + layers[t * (numberOfStepsX + 1) + x + 1] -
					2 * layers[t * (numberOfStepsX + 1) + x]
					);
		}
